 ...
 * 
 */
    const getFilesNames = (tplsFolder)=>(fs.readdirSync(path.join(process.cwd(),tplsFolder)))
/**
 ...
 * 
 */
    const getFilesForActions = (names, templatePath)=>{
        if(typeof names === "string"){
          return getFilesNames(names, templatePath)
        }
        return names
      }
      ...
      const actionsCreator = (data,type, options) =>{    
        ...
        const templatePath = data.other_template ? `${templatesPath}${data.other_template}/`: typeof files === "string" ?  `${templatesPath}${files}`: templatesPath;
